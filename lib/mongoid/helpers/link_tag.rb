...
module Mongoid
  module Helpers
    ...
    # with underscored class name (besides the "LTag" class. LTag is an abstract
    ...
    class LinkTag
      ...
      def partial_path
        "#{@views_prefix}/pagination/#{@theme}/#{self.class.name.demodulize.underscore}".gsub('//', '/')
      end
    end
    ...
    class PrevPage < LinkTag
      ...
      def initialize(template, params: {}, param_name: nil, theme: nil, views_prefix: nil, **options) #:nodoc:
        ...
        if params && params.respond_to?(:to_unsafe_h)
          Logger.warn 'Explicitly passing params to helpers could be omitted.'
